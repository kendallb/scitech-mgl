...
size_t  _CDECL stub_lseek(int _fildes, size_t _offset, int _whence);
...
int _CDECL stub_system(const char *_s)
{
#if defined(__WINDOWS__) || defined(__WIN32_VXD__) || defined(__NT_DRIVER__) || defined(__SMX32__) || defined(__RTTARGET__) || defined(__WINCE__)
    (void)_s;
    return -1;
#else
    return system(_s);
...
#elif   defined(__NT_DRIVER__) || defined(__WINCE__)
...
ulong _CDECL _OS_getcurrentdate(void)
{
    ...
    systm.wDayOfWeek    = 0;
    SystemTimeToFileTime(&systm, &reftm);
    memcpy(&llref,&reftm,sizeof(__int64));

    GetSystemTime(&systm);
    SystemTimeToFileTime(&systm, &abstm);
    memcpy(&llsys,&abstm,sizeof(__int64));

    return (ulong)( (llsys - llref) / ((_int64)24 * (_int64)3600 * (_int64)10000000) );
}
#else
/* Return the current date in days since 1/1/1980 */
ulong _CDECL _OS_getcurrentdate(void)
...
{
#if defined(__WIN32_VXD__) || defined(__NT_DRIVER__) || defined(__SMX32__) || defined(__RTTARGET__) || defined(__WINCE__)
    return -1;
#else
    return raise(sig);
#endif
