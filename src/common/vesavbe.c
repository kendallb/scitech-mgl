...
static char *VBE_copyStrToLocal(char *p,ulong realPtr,char *max)
/****************************************************************************
*
...
****************************************************************************/
{
    ...
    v = PM_mapRealPointer((uint)(realPtr >> 16), (uint)(realPtr & 0xFFFF));
    while (*v != 0 && p < max)
        *p++ = *v++;
    *p++ = 0;
    return p;
...
static void VBE_copyShortToLocal(ushort *p,ulong realPtr)
/****************************************************************************
*
...
****************************************************************************/
{
    ...
    strncpy(vgaInfo16.VESASignature,"VBE2",4);
    VBE_callESDI(&regs, &vgaInfo16, sizeof(vgaInfo16));
    if (regs.x.ax != VBE_SUCCESS)
        return 0;
    if (strncmp(vgaInfo16.VESASignature,"VESA",4) != 0)
        return 0;

    /* Convert little endian values as necessary */
    vgaInfo->VESAVersion        = PM_getLE_16(vgaInfo16.VESAVersion);
    vgaInfo->Capabilities       = PM_getLE_32(vgaInfo16.Capabilities);
    vgaInfo->TotalMemory        = PM_getLE_16(vgaInfo16.TotalMemory);
    vgaInfo->OemSoftwareRev     = PM_getLE_16(vgaInfo16.OemSoftwareRev);
     ...
     */
    if (vgaInfo->VESAVersion >= 0x200 && vgaInfo16.OemVendorNamePtr == 0)
     ...
     */
    p2 = VBE_copyStrToLocal(localBuf,PM_getLE_32(vgaInfo16.OemStringPtr),MAX_LOCAL_BUF);
    vgaInfo->OemStringPtr = localBuf;
    if (vgaInfo->VESAVersion >= 0x200 && (strcmp(vgaInfo->OemStringPtr,"ATI ATOMBIOS") != 0)) {
        /* Copy OEM vendor information */
        p = VBE_copyStrToLocal(p2,PM_getLE_32(vgaInfo16.OemVendorNamePtr),MAX_LOCAL_BUF);
        vgaInfo->OemVendorNamePtr = p2;
        p2 = VBE_copyStrToLocal(p,PM_getLE_32(vgaInfo16.OemProductNamePtr),MAX_LOCAL_BUF);
        vgaInfo->OemProductNamePtr = p;
        p = VBE_copyStrToLocal(p2,PM_getLE_32(vgaInfo16.OemProductRevPtr),MAX_LOCAL_BUF);
        vgaInfo->OemProductRevPtr = p2;
        VBE_copyShortToLocal((ushort*)p,PM_getLE_32(vgaInfo16.VideoModePtr));
        vgaInfo->VideoModePtr = (ushort*)p;
        }
    else {
        if (vgaInfo->VESAVersion >= 0x200) {
            /* Some BIOS'es (ATI Radeon X800) do not properly return the
             * Oem vendor information, so we NULL out those strings.
             */
            vgaInfo->OemVendorNamePtr = "";
            vgaInfo->OemProductNamePtr = "";
            vgaInfo->OemProductRevPtr = "";
            }
        VBE_copyShortToLocal((ushort*)p2,PM_getLE_32(vgaInfo16.VideoModePtr));
     ...
     */
    haveGeodeGX2 = false;
    if (vgaInfo->VESAVersion >= 0x200 && ((strcmp(vgaInfo->OemStringPtr,"GEODE GX2") == 0)
            || (strcmp(vgaInfo->OemProductNamePtr,"GEODE GX2") == 0)))
        haveGeodeGX2 = true;

    /* Check for Matrox G400 cards which claim to be VBE 3.0
     * compliant yet they don't implement the refresh rate control
     * functions.  Same for ATI ATOM BIOS.
     */
    if (vgaInfo->VESAVersion >= 0x300 &&
        (strcmp(vgaInfo->OemProductNamePtr,"Matrox G400") == 0 ||
         strcmp(vgaInfo->OemStringPtr,"ATI ATOMBIOS") == 0))
...
****************************************************************************/
{
    ...
    /* Convert little endian values as necessary */
    PM_convLE_16(modeInfo->ModeAttributes);
    PM_convLE_16(modeInfo->WinGranularity);
    PM_convLE_16(modeInfo->WinSize);
    PM_convLE_16(modeInfo->WinSize);
    PM_convLE_16(modeInfo->WinASegment);
    PM_convLE_16(modeInfo->WinBSegment);
    PM_convLE_16(modeInfo->BytesPerScanLine);
    PM_convLE_16(modeInfo->XResolution);
    PM_convLE_16(modeInfo->YResolution);
    PM_convLE_32(modeInfo->PhysBasePtr);
    PM_convLE_16(modeInfo->LinBytesPerScanLine);
    PM_convLE_32(modeInfo->MaxPixelClock);

    /* Bail if the mode is not available */
     ...
     * be recognised as 15 bits per pixel modes. Also fixes code from
     * stupid BIOS developers at ATI with their Radeon X800 crapola BIOS.
     */
    if (bits == 16 && modeInfo->RedMaskSize == 5 && modeInfo->GreenMaskSize == 5
            && modeInfo->BlueMaskSize == 5)
...
****************************************************************************/
{
    ...
    if (state->VBEVersion < 0x200 && mode < 0x100) {
        ...
        /* Convert CRTC structure to little endian */
        PM_convLE_16(crtc->HorizontalTotal);
        PM_convLE_16(crtc->HorizontalSyncStart);
        PM_convLE_16(crtc->HorizontalSyncEnd);
        PM_convLE_16(crtc->VerticalTotal);
        PM_convLE_16(crtc->VerticalSyncStart);
        PM_convLE_16(crtc->VerticalSyncEnd);
        PM_convLE_32(crtc->PixelClock);
        PM_convLE_16(crtc->RefreshRate);

        /* Set the display mode with refresh control */
        regs.x.ax = 0x4F02;
        ...
        /* Convert CRTC structure back again on exit */
        PM_convLE_16(crtc->HorizontalTotal);
        PM_convLE_16(crtc->HorizontalSyncStart);
        PM_convLE_16(crtc->HorizontalSyncEnd);
        PM_convLE_16(crtc->VerticalTotal);
...
****************************************************************************/
{
     ...
     */
    if (regs.x.cx < pixelsPerLine) {
        int i = 1;
        while ((regs.x.cx < pixelsPerLine) && (regs.x.ax == VBE_SUCCESS) && (i < 256)) {
            regs.x.ax = 0x4F06;
            regs.h.bl = 0;
            regs.x.cx = pixelsPerLine + i++;
            PM_int86(0x10,&regs,&regs);
            }
        }
