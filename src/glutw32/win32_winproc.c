...
__glutWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  int button = -1;
  static int    numWindows = 0; /* Open window counter. (DaveM) */
  ...
  case WM_CREATE:
    ...
    return 0;
  case WM_CLOSE:
#if 0 /* Lazy mechanism to exit WIN32 thread (DaveM) */
    if (__glutExitFunc) {
      __glutExitFunc(0);
    }
//    exit(0);
...
//    return 0;
#endif
#if 1 /* Standard mechanism to exit WIN32 thread (DaveM) */
    /* Don't quit until last opened window has been destroyed. */
    if (--numWindows == 0)
       PostQuitMessage(0);
#endif
    break;
...
#if 0 /* Intercept ESCAPE key to prevent handling by application (DaveM) */
      if (wParam == VK_ESCAPE)
        return 0;
#endif
...
#if 1 /* Intercept ESCAPE key to exit via window close event (DaveM) */
    if (wParam == VK_ESCAPE) {
#if 1 /* Not Win2K-friendly for some unknown reason (DaveM) */
