...
static char *getdirent(char *dir)
{
  ...
  {                                    /* get first entry */
    hdir = HDIR_SYSTEM;
    count = 1;
    done = DosFindFirst((PSZ)dir, &hdir, attributes, &find, sizeof(find), &count);
...
int IsFileSystemFAT(char *dir)
{
  ...
  if (!DosQueryFSAttach((PSZ)bName, 0, FSAIL_QUERYNAME, (PVOID) pData, &cbData))
...
#ifdef __32BIT__
  FILESTATUS3 fs;
  return DosQueryPathInfo((PSZ)name, 1, &fs, sizeof(fs)) ? -1 : fs.attrFile;
#else
  USHORT mode;
...
void SetFileTime(char *path, ulg stamp)
{
  ...
  if (DosQueryPathInfo((PSZ)path, FIL_STANDARD, (PBYTE) &fs, sizeof(fs)))
    return;
  ...
  DosSetPathInfo((PSZ)path, FIL_STANDARD, (PBYTE) &fs, sizeof(fs), 0);
}
...
char *GetLongNameEA(char *name)
{
  ...
  if (DosQueryPathInfo((PSZ)name, FIL_QUERYEASFROMLIST,
  ...
  if (fealst.cbValue > 4 && fealst.eaType == 0xFFFD)
  {
    ...
    for (ptr = (char *)fealst.szValue; *ptr; ptr++)
      if (*ptr == '/' || *ptr == '\\')
        *ptr = '!';
                        ...
                        char **cbufptr, size_t *csize)
{
  ...
  *size = *csize = 0;
  ...
  if (DosQueryPathInfo((PSZ)szName, FIL_QUERYEASIZE, (PBYTE) &fs, sizeof(fs)))
    return;
  ...
  if (DosQueryPathInfo((PSZ)szName, FIL_QUERYEASFROMLIST,
                       (PBYTE) &eaop, sizeof(eaop)))
  {
    unalloc(pDENA);
    unalloc(pGEAlist);
