...
#ifdef __WATCOMC__
// Since 11.0 has trouble with dynamically imported 16-bit functions,
// why not call the 32-bit variants instead?
APIRET (APIENTRY * NetAccessGetInfo)(PCHAR pszServer, PCHAR pszResource,
  USHORT sLevel, PVOID pbBuffer, USHORT cbBuffer, PVOID pcbTotalAvail);
APIRET (APIENTRY * NetAccessSetInfo)(PCHAR pszServer, PCHAR pszResource,
  USHORT sLevel, PVOID pbBuffer, USHORT cbBuffer, USHORT sParmNum);
APIRET (APIENTRY * NetAccessAdd)(PCHAR pszServer,
  USHORT sLevel, PVOID pbBuffer, USHORT cbBuffer);
#else
APIRET16 (APIENTRY16 * NetAccessGetInfo)(PCHAR pszServer, PCHAR pszResource,
  USHORT sLevel, PVOID pbBuffer, USHORT cbBuffer, PVOID pcbTotalAvail);
APIRET16 (APIENTRY16 * NetAccessSetInfo)(PCHAR pszServer, PCHAR pszResource,
  USHORT sLevel, PVOID pbBuffer, USHORT cbBuffer, USHORT sParmNum);
APIRET16 (APIENTRY16 * NetAccessAdd)(PCHAR pszServer,
  USHORT sLevel, PVOID pbBuffer, USHORT cbBuffer);
#endif
...
static BOOL acl_init(void)
{
  ...
  if (DosLoadModule((PSZ)buf, sizeof(buf), (PSZ)"NETAPI32", &netapi))
    return FALSE;

  if (DosQueryProcAddr(netapi, 0, (PSZ)"Net32AccessGetInfo", (PFN *) &_NetAccessGetInfo) ||
      DosQueryProcAddr(netapi, 0, (PSZ)"Net32AccessSetInfo", (PFN *) &_NetAccessSetInfo) ||
      DosQueryProcAddr(netapi, 0, (PSZ)"Net32AccessAdd", (PFN *) &_NetAccessAdd))
    return FALSE;
#else
  if (DosLoadModule((PSZ)buf, sizeof(buf), (PSZ)"NETAPI", &netapi))
    return FALSE;

  if (DosQueryProcAddr(netapi, 0, (PSZ)"NETACCESSGETINFO", (PFN *) &_NetAccessGetInfo) ||
      DosQueryProcAddr(netapi, 0, (PSZ)"NETACCESSSETINFO", (PFN *) &_NetAccessSetInfo) ||
      DosQueryProcAddr(netapi, 0, (PSZ)"NETACCESSADD", (PFN *) &_NetAccessAdd))
    return FALSE;
#endif
...
static void acl_mkpath(char *buffer, const char *source)
{
  ...
  else
  {
    if (cwd[0] == 0)
    {
      ...
      cwd[2] = '\\';
      cwdlen = sizeof(cwd) - 3;
      DosQueryCurrentDir(0, (PSZ)(cwd + 3), &cwdlen);
      cwdlen = strlen(cwd);
    }
