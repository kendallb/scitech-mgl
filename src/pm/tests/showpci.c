...
static void EnumeratePCI(void)
{
    ...
    for (i = 0; i < NumBridges; i++) {
        ...
        printf("%04X:%04X %02X %02X %02X %04X %04X %08X %08X %08X %08X %04X\n",
            ...
            info->u.type1.SecondaryBusNumber,
    ...
    if (NumPCI == -1) {
        ...
        for (i = 0,NumDevices = 1,DeviceIndex[0] = -1; i < NumPCI; i++) {
            if (PCI_IS_DISPLAY_CLASS(&PCI[i])) {
                if ((DeviceIndex[0] == -1) && (PCI[i].Command & 0x3) == 0x3)
                    DeviceIndex[0] = i;
                else
                ...
                if (PCI[i].slot.p.Bus != 0) {
                    ...
                    for (j = 0; j < NumBridges; j++) {
                        ...
                        if (info->u.type1.SecondaryBusNumber == PCI[i].slot.p.Bus) {
                            AGPBridge = info;
                            break;
                            }
                        }
        ...
        // If the primary card has VGA I/O resources disabled, we
        // will renumber all the cards so the primary card becomes the
        // first one on the bus in bus order (nearly always the PCI card).
        if (DeviceIndex[0] == -1 && NumDevices > 1) {
            for (i = 1; i < NumDevices; i++)
                DeviceIndex[i-1] = DeviceIndex[i];
            NumDevices -= 1;
            }
