...
int PMAPI PM_haveCauseway(void);
    ...
    void *context)
{
    ...
    *((ulong*)&handle->thunk[GLOBALS_START+16]) = IRQ;
     ...
     * so we have to do this manually. CauseWay does, so if CauseWay is our
     * DOS extender we let it handle real mode interrupt passup.
     */
