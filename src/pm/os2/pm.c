...
/* More defines missing from old Toolkit headers */
#ifndef PT_WINDOWABLEVIO
...
/* VAC++ is picky and so are other compilers - only in a different way      */
#ifdef __IBMC__
...
void PMAPI PM_init(void)
{
    if (!lowMem && !inPMInit) {
        ...
        HMODULE     hModSDDPMI, hModVideoPMI;
        ...
        /* If SDDPMI is loaded, we'll need the semaphores */
        if (DosQueryModuleHandle((PSZ)"SDDPMI.DLL",&hModSDDPMI) == NO_ERROR) {
            DosLoadModule(NULL, 0, (PSZ)"SDDPMI.DLL", &hModSDDPMI);
            ...
            /* Also make sure VIDEOPMI is mapped to this process */
            DosLoadModule(NULL, 0, (PSZ)"VIDEOPMI.DLL", &hModVideoPMI);
...
{
    static char path[CCHMAXPATH];
    if (getenv("SNAP_CONFIG_PATH") != NULL)
        return getenv("SNAP_CONFIG_PATH");
...
void PMAPI PM_sleep(
    ulong milliseconds)
{
    /* The yield variant (milliseconds == 0) may be called from Ring 2 where
     * DosSleep will crash; additionally breaking into busy looping programs
     * is much simpler without DosSleep, and on OS/2 it won't buy us much
     * anyway since the graphics subsystem is already semaphore protected.
     */
    if (!milliseconds)
        return;
...
/****************************************************************************
REMARKS:
Returns true if process is VIO or PM.
****************************************************************************/
ibool PMAPI PM_runningInAWindow(void)
{
    ...
    DosGetInfoBlocks(&ptib, &ppib);
    return (ppib->pib_ultype == PT_WINDOWABLEVIO) || (ppib->pib_ultype == PT_PM);
}
