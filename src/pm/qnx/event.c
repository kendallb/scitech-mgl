...
static void _EVT_pumpMessages(void)
{
    ...
    while ((numkeys = read(kbd_fd, buf, sizeof buf)) > 0) {
        for (i = 0; i < numkeys; i++) {
...
#if __OLD_NTO__
        // The following code does not work. It is supposed to work on Neutrino,
        // but fails on QNX 6.3.x because there's no /dev/keyboard0. The above
        // keyboard code is the same as we use on QNX 4 and reads raw scan codes
        // from /dev/kbd -- and works.
...
/****************************************************************************
PARAMETERS:
mouseMove   - Callback function to call whenever the mouse needs to be moved
...
#ifdef __QNXNTO__
#if __OLD_NTO___
    // Mouse support is not functional at the moment. The inputtrap
    // executable exists, but isn't on PATH and seems to need a 'query'
    // parameter to do what the following code expects. Even then it
    // won't deliver proper data. It is not clear where /dev/mouse0 
...
#endif
    if ((kbd_fd = open("/dev/kbd", O_RDONLY|O_NONBLOCK)) < 0) {
        perror("/dev/kbd");
        PM_fatalError("Could not open /dev/kbd");
        }
#else
