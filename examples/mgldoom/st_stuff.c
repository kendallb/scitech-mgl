...
void ST_refreshBackground(void)
{
    ...
    if (st_statusbaron)
    {
        V_DrawScaledPatch(ST_X, 0, BG, sbar);

        if (netgame)
            V_DrawScaledPatch(ST_FX, 0, BG, faceback);
...
void ST_Init (void)
{
    ...
    width = ST_WIDTH * SCREENWIDTH / 320 + 1;
    height = ST_HEIGHT * SCREENHEIGHT / 200 + 1;

    veryfirsttime = 0;
    ST_loadData();
    screens[4] = (byte *) Z_Malloc(width*height, PU_STATIC, 0);
}
