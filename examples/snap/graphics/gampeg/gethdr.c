...
static void getseqhdr()
{
  ...
  if ((load_intra_quantizer_matrix = getbits(1)) != 0)
  {
    for (i=0; i<64; i++)
  ...
  if ((load_non_intra_quantizer_matrix = getbits(1)) != 0)
  {
    for (i=0; i<64; i++)
...
static void quant_matrix_extension()
{
  ...
  if ((load_intra_quantiser_matrix = getbits(1)) != 0)
  {
    for (i=0; i<64; i++)
    {
  ...
  if ((load_non_intra_quantiser_matrix = getbits(1)) != 0)
  {
    for (i=0; i<64; i++)
    {
  ...
  if ((load_chroma_intra_quantiser_matrix = getbits(1)) != 0)
  {
    for (i=0; i<64; i++)
      ld->chroma_intra_quantizer_matrix[zig_zag_scan[i]] = getbits(8);
  }

  if ((load_chroma_non_intra_quantiser_matrix = getbits(1)) != 0)
  {
    for (i=0; i<64; i++)
...
static void sequence_scalable_extension()
{
  ...
  if (ld->scalable_mode==SC_SPAT)  
    error("spatial scalability not implemented\n");
