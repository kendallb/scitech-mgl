...
*
* Language:     ANSI C
* Environment:  Win32, OS/2, Linux
*
* Description:  Include file for the SciTech Portability Manager 32-bit
*               helper VxD for Windows 9x, ring-0 device driver for Windows
*               NT/XP, kernel module for Linux, and the 16-bit ring-0
...
#if !defined(__OS2_DRIVER__) && !defined(__KERNEL__)
 ...
 * kernel module for Linux. Similar to that used for the DOS/Win32 version.
 * Extra bit fields encoded for parameter size, type, and transfer direction
 * as recommended for modern Linux kernels.
 */

#include <asm/ioctl.h>
#define IOCTL_MAGIC                 'F'
#define IOCTL_SIZE                  ulong
#define PMHELP_CTL_CODE(name,value) \
    PMHELP_##name = _IOWR(IOCTL_MAGIC,value,IOCTL_SIZE)
