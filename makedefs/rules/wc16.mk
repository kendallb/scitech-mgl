...
%$O: %.c ; $(CC) @$(mktmp $(_CFLAGS)) $<
%$O: %$P ; $(CPP) @$(mktmp $(_CFLAGS)) $<
%$O: %$A ; $(AS) @$(mktmp $(_ASFLAGS)) $(<:s,/,\)
...
.IF $(BUILD_DLL)
%$E: ;
        @trimlib $(mktmp $(_LDFLAGS) OP quiet SYS windows_dll\nN $@\nF $(&:t",":s/\/\\)\nLIBR $(EXELIBS:t",")) $*.lnk
        rclink $(LD) $(RC) $@ $*.lnk
        @$(RM) -S $(mktmp $*.lnk)
.ELSE
%$E: ;
        @trimlib $(mktmp $(_LDFLAGS) OP quiet SYS windows\nN $@\nF $(&:t",":s/\/\\)\nLIBR $(EXELIBS:t",")) $*.lnk
        rclink $(LD) $(RC) $@ $*.lnk
        @$(RM) -S $(mktmp $*.lnk)
.ENDIF
.ELSE
%$E: ;
        @trimlib $(mktmp OP quiet SYS dos\nN $@\nF $(&:t",":s/\/\\)\nLIBR $(PMLIB) $(EXELIBS:t",")) $*.lnk
        $(LD) $(_LDFLAGS) @$*.lnk
        @$(RM) -S $(mktmp $*.lnk)
.ENDIF
